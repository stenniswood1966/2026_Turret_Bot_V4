...
public class FeederSubsystem extends SubsystemBase {
  ...
  private static TalonFX motor1 = new TalonFX(constants.kg_FeederSubsystem.k_Motor1ID, constants.kg_FeederSubsystem.k_Motor1CANBus);
  ...
  public FeederSubsystem() {
    ...
    fx_cfg.CurrentLimits.StatorCurrentLimitEnable = constants.kg_FeederSubsystem.k_Motor1StatorCurrentLimitEnable;
    fx_cfg.CurrentLimits.SupplyCurrentLimit = constants.kg_FeederSubsystem.k_Motor1SupplyCurrentLimit;
    fx_cfg.CurrentLimits.SupplyCurrentLimitEnable = constants.kg_FeederSubsystem.k_Motor1StatorCurrentLimitEnable;
    fx_cfg.CurrentLimits.SupplyCurrentLowerLimit = constants.kg_FeederSubsystem.k_Motor1SupplyCurrentLowerLimit;
    fx_cfg.CurrentLimits.SupplyCurrentLowerTime = constants.kg_FeederSubsystem.k_Motor1SupplyCurrentLowerTime;
  ...
  private void feedMethod(){
    if (!killFlag) {
      motor1.setControl(new DutyCycleOut(constants.kg_FeederSubsystem.k_FeedPower));
    }
  }
  ...
  private void antiJamMethod(){
    if (!killFlag) {
      motor1.setControl(new DutyCycleOut(constants.kg_FeederSubsystem.k_AntiJamPower));
    }
  }

  public Trigger t_feederOn(AngularVelocity threshold){
    return new Trigger(() -> {
      return motor1MotorVelocity.getValue().gt(threshold);
    });
  }
}
